...
class VectorizeSummaryStep(PipelineStep):
    name = "vectorize_summary"
    ...
    async def execute(self, ctx: PipelineContext) -> PipelineContext:
        summary_text = ctx.page_summary if isinstance(ctx.page_summary, str) else ""
        if not summary_text:
            logger.warning("vectorize_summary_skip", reason="no summary")
            return ctx

        if not self.chunker or not self.embedder or not self.vector_store:
            logger.warning("vectorize_summary_skip", reason="vector services not configured")
            return ctx
        ...
        chunks = self.chunker.chunk(summary_text)
